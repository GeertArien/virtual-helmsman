...
def _client(tmp_path: Path, stub: _StubClient) -> tuple[TestClient, IngestionEngine]:
    ...
    app = FastAPI()
    app.include_router(
        create_review_router(_cfg(tmp_path), engine=engine)
    )
...
def test_pdf_extraction_failure_writes_audit_row(tmp_path: Path) -> None:
    ...
    app = FastAPI()
    app.include_router(create_review_router(_cfg(tmp_path), engine=engine))
...
def test_create_review_router_exposes_engine(tmp_path: Path) -> None:
    cfg = _cfg(tmp_path)
    router = create_review_router(cfg, llm_model="m")
    # The router exposes the engine for app-shutdown cleanup.
